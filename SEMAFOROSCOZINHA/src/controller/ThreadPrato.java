...
public class ThreadPrato extends Thread {
	...
	private long tCozimento;
	private Semaphore semaforoEntrega;

	public ThreadPrato(int id, Semaphore semaforoEntrega) {
		this.idThread = id;
	...
	private void metodo() {
		if (idThread % 2 == 1) {
			tCozimento = (long) (Math.random() * 300) + 500;
			...
			while (tempoNoFogo < tCozimento) {
				try {
					sleep(1000);
					tempoNoFogo += 100;
					percentCozimento = (tempoNoFogo / tCozimento);
			...
			tCozimento = (long) (Math.random() * 600) + 600;
